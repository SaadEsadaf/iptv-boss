...
const FR = {
  welcomeBack: 'Bonjour', createAccount: 'Créer un Compte',
  signIn: 'Connexion', signUp: 'Inscription',
  fullName: 'Nom complet', emailAddress: 'Adresse email', password: 'Mot de passe',
  processing: 'Traitement...',
  signInBtn: 'Se Connecter', createAccountBtn: 'Créer un Compte',
  orContinue: 'ou continuer avec',
  googleConfig: 'Google Sign-In — configurez le Client ID Google OAuth dans les paramètres Admin',
  signInApple: 'Se connecter avec Apple',
  appleConfig: 'Apple Sign-In — configurez dans les paramètres Admin',
  ...
  appleCancelled: 'Connexion Apple annulée ou échouée',
  authFailed: 'Authentification échouée',
  appleNotConfigured: 'Connexion Apple non configurée',
}
...
export default function AuthModal({ settings, onAuth, onClose }) {
  ...
  function handleGoogleResponse(response) {
    ...
    fetch('/api/auth/google', {
      ...
      else setError(data.error || t('googleFailed'))
    }).catch(() => setError(t('networkError'))).finally(() => setLoading(false))
  }
  ...
  async function handleAppleSignIn() {
    ...
    try {
      ...
      if (!s.appleClientId) { setError(t('appleNotConfigured')); return }
      const script = document.createElement('script')
      ...
      if (data.token) { localStorage.setItem('user_token', data.token); onAuth(data.user, data.token) }
      else setError(data.error || t('appleFailed'))
    } catch (e) { setError(t('appleCancelled')) }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <form onSubmit={handleSubmit}>
          ...
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder={t('emailAddress')} required
            style={{ width: '100%', padding: '12px 14px', borderRadius: 8, border: '1px solid #333', background: '#0f0f0f', color: '#fff', fontSize: 14, outline: 'none', boxSizing: 'border-box', marginBottom: 10 }} />
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} placeholder={t('password')} required minLength={6}
          ...
          <button type="submit" disabled={loading} style={{
            ...
            {loading ? t('processing') : tab === 'login' ? t('signInBtn') : t('createAccountBtn')}
          </button>
        </form>

        <div style={{ display: 'flex', alignItems: 'center', gap: 12, margin: '16px 0' }}>
          <div style={{ flex: 1, height: 1, background: '#2a2a2a' }} />
          <span style={{ color: '#666', fontSize: 12 }}>{t('orContinue')}</span>
          <div style={{ flex: 1, height: 1, background: '#2a2a2a' }} />
        </div>
        ...
        <button onClick={handleAppleSignIn} disabled={!settings?.appleClientId || loading} style={{
          ...
          {settings?.appleClientId ? t('signInApple') : t('appleConfig')}
        </button>
